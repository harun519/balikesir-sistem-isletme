...
type HealthPayload={ok?:boolean;database?:boolean;lastDataAt?:string|null;lastBackupAt?:string|null;lastBackupName?:string|null;backupScheduleConfigured?:boolean;latencyMs?:number;checkedAt?:string;error?:string};
...
export default async function Saglik(){
  const rows=await Promise.all(systems.map(async s=>{
    ...
    const cronOk=health.data?.backupScheduleConfigured===true;
    return {...s,app,health,db,backupOk,cronOk};
  }));
  const allOk=rows.every(r=>r.app.ok&&r.db&&r.backupOk&&r.cronOk);
  const checked=new Date().toLocaleString("tr-TR",{timeZone:"Europe/Istanbul"});
  const issueCount=rows.reduce((n,r)=>n+(!r.app.ok?1:0)+(!r.db?1:0)+(!r.backupOk?1:0)+(!r.cronOk?1:0),0);
  return <main className="page"><div className="wrap">
    ...
    <section className="summary"><Summary label="Uygulama" value={`${rows.filter(r=>r.app.ok).length}/3`} ok={rows.every(r=>r.app.ok)}/><Summary label="Veritabanı" value={`${rows.filter(r=>r.db).length}/3`} ok={rows.every(r=>r.db)}/><Summary label="Güncel Yedek" value={`${rows.filter(r=>r.backupOk).length}/3`} ok={rows.every(r=>r.backupOk)}/></section>
    <section className="grid">{rows.map(r=><article className="card" key={r.name} style={{"--tone":r.tone} as React.CSSProperties}>
      <div className="head"><div className="ico">{r.icon}</div><div><h2>{r.name}</h2><span>{r.url.replace("https://","")}</span></div><span className={`bigstate ${r.app.ok&&r.db&&r.backupOk&&r.cronOk?"ok":"bad"}`}>{r.app.ok&&r.db&&r.backupOk&&r.cronOk?"SAĞLIKLI":"KONTROL"}</span></div>
      <div className="checks"><Status label="Vercel / Uygulama" ok={r.app.ok} detail={r.app.ok?`${r.app.ms} ms yanıt` : "Uygulamaya erişilemiyor"}/><Status label="Supabase / Veri" ok={r.db} detail={r.db?`Bağlantı aktif · ${r.health.data?.latencyMs??r.health.ms} ms`:r.health.data?.error||"Veri bağlantısı yok"}/><Status label="Son Veri Güncellemesi" ok={r.db} detail={fmtDate(r.health.data?.lastDataAt)} neutral/><Status label="Otomatik Yedekleme" ok={r.cronOk} detail={r.cronOk?"Zamanlayıcı güvenli şekilde hazır":"CRON_SECRET tanımlı değil"}/><Status label="Son Otomatik Yedek" ok={r.backupOk} detail={`${fmtDate(r.health.data?.lastBackupAt)}${age(r.health.data?.lastBackupAt)?` · ${age(r.health.data?.lastBackupAt)}`:""}`}/><div className="row"><div><b>Yedek Planı</b><small>{r.backup} · son yedekler geri yüklenebilir</small></div><span className="pill backup">GÜNLÜK</span></div></div>
      <div className="actions"><a className="open" href={r.url} target="_blank" rel="noreferrer">Uygulamayı Aç →</a></div>
    </article>)}</section>
    <div className="note"><b>Kontrol mantığı:</b> Uygulama adresi canlı olarak açılır, her uygulamanın kendi güvenli sağlık servisi veritabanına erişir ve son yedek tarihini okur. Otomatik zamanlayıcı hazır değilse veya son başarılı yedek 24 saati aşarsa sistem kırmızı uyarıya düşer. Bu ekran hiçbir servis anahtarını tarayıcıya göndermez.</div>
  </div><style>{`
