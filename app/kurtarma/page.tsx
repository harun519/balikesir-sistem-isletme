...
type Health={reachable?:boolean;database?:boolean;lastBackupAt?:string|null;lastBackupName?:string|null;lastDataAt?:string|null;backupScheduleConfigured?:boolean;ms?:number;error?:string};
...
function backupFresh(v?:string|null){if(!v)return false;const t=new Date(v).getTime();return Number.isFinite(t)&&Date.now()-t<=24*3600000}
...
export default function Kurtarma(){
 ...
 return <main className="page"><div className="wrap">
   ...
   <div className="grid">{items.map(x=><button type="button" className={`card ${selected===x.key?"selected":""}`} key={x.key} onClick={()=>void choose(x.key)}><div className="cardHead"><h2>{x.icon} {x.name}</h2><span className={`state ${x.data?.reachable?"ok":"bad"}`}>{x.data?.reachable?"ÇALIŞIYOR":"KONTROL"}</span></div><div className="row"><span>Servis</span><b>{x.data?.reachable?"Erişilebilir":"Ulaşılamıyor"}</b></div><div className="row"><span>Veritabanı</span><b>{x.data?.database===undefined?"Bilgi yok":x.data.database?"Bağlı":"Bağlantı yok"}</b></div><div className="row"><span>Son yedek</span><b>{fmt(x.data?.lastBackupAt)}</b></div><div className="row"><span>Yedek adı</span><b className="file">{x.data?.lastBackupName||"Bilgi yok"}</b></div><div className="row"><span>Yedek durumu</span><b className={backupFresh(x.data?.lastBackupAt)?"fresh":"stale"}>{backupFresh(x.data?.lastBackupAt)?"GÜNCEL":"GÜNCEL DEĞİL"}</b></div><div className="tap">{selected===x.key?"Yedekleri kapat ↑":"Yedekleri listele ↓"}</div></button>)}</div>
